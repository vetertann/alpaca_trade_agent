...
SIZING_POSTURE_GUIDANCE = {
    ...
    "terminal_push": (
        ...
        "the first new entry, compare at least one true bullish debit call spread "
        "(`vertical_call` with the lower-strike call bought and the higher-strike "
        "call sold), not a positive-delta straddle relabelled as bullish; also compare "
        "one bearish direction-led candidate, and suitable volatility-led "
        "alternatives. Every "
        "active-session startup, anchor, rebaseline, deployment-floor, or portfolio-"
        "build cycle must perform this bounded comparison before `decision.no_trade`. "
        "Existing exposure or general caution is not a substitute: preview the "
        "finalists and let the host quantify the actual constraint. No-trade is valid "
        "only after the scan finds no robust finalist, a fresh reprice destroys the "
        "edge, market data are unavailable, or a named host gate refuses admission. "
        "Keep candidate collections keyed by underlying (or re-enumerate the exact "
        "finalist); never depend on a loop-local candidate map overwritten by the "
        "next underlying. **Bound the expensive funnel:** request at most 48 rows "
        "from each enumeration, select at most 12 candidates total across both "
        "underlyings before calling `vol.evaluate_many`, and reserve that shortlist "
        "for one true bull call, one bearish comparator, one volatility alternative, "
        "then the best low-friction rows. Never evaluate every candidate in every "
        "expiry/family bucket; fully rescore only 2–3 finalists. For the single best "
        "genuinely different candidate with "
        "three positive measures, stable rank, fresh edge after friction, liquid "
        "quotes, and no conflicting directional evidence, the host target is 20% of "
        "equity maximum loss and may use up to the configured 25% evidence and "
        "single-position ceiling. Request at least 85% of that target when headroom "
        "exists. Prefer an "
        ...
        "positive after adverse IV repricing. **Maximum loss is only a safety "
        "boundary, not the tournament objective.** `vol.evaluate` already values "
        "each candidate at its labelled `evaluation_at`, which is the official "
        "scoring horizon for expirations after the mark. For at most three robust "
        "finalists, call `trading.preview` using the intended terminal risk budget, "
        "then multiply every `expected_profit_by_measure` value by the previewed "
        "whole-contract quantity. Compare the resulting weakest and median expected "
        "**dollar** P&L at the scoring mark. Select for the largest robust executable "
        "terminal-equity impact, not the largest premium, maximum loss, nominal "
        "quantity, or distant-expiry payoff. A later-dated option is eligible only "
        "when its scoring-horizon dollar P&L beats the nearer alternatives after "
        "spread friction; reject a large-max-loss position whose value is expected "
        "to change little before the mark. Also print the selected quantity, weakest "
        "and median expected terminal P&L, fresh friction, and adverse portfolio "
        "scenario before staging so the decision is auditable. The host permits only "
        "one terminal-sized submission and will request reconsideration when "
        "excellent evidence is materially undersized despite sufficient headroom. "
        "After that submission, ordinary robust entries revert to a 4% ceiling. "
        "Never promote weak evidence, duplicate exposure, average down, or trade "
        "merely to spend risk; no-trade remains correct when nothing qualifies."
    ),
}
